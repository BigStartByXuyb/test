...
| 需求 | 改哪里 |
|---|---|
| 新增组件/变体模板 | 按 `skills/mastergo-iocontrol-document-format/SKILL.md` 的「新增/修改映射的同步清单」整批完成：映射表 + 映射文档（变体必须写成 `MasterGo 变体：…` 或 `### 固定模板：…` 标题，审计按这些结构化写法解析）+ 回归用例，最后跑覆盖审计与全量回归（哪些审计字段会拦下改动，以 `audit-mtslg-feishu-map.js` 的退出判据为唯一真值源，本表不复制清单）。**新增的是"族"（带 `variants` 的新 `*Templates`）时另有两步不可省**：在 `gen-mtslg-mapping-from-dsl.js` 里写出该族的发射分支并登记进 `SUPPORTED_TEMPLATE_FAMILIES`，然后过 `template-family-coverage.test.js`——它会把两侧差集点名报出（只登记不写分支同样会失败） |
