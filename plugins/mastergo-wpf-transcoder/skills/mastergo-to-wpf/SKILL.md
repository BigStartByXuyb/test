...
- 运行登记表：`<项目>/Generated/runs/<Target>/run.json`，规则是「**产出即登记、消费只按登记取、未登记的旧同名文件一律拒绝**」；清单里的采集输入（`dslPath` / `visibilityPath` / `svgPath`）都从登记表解析并校验 `sha256`。断点续跑用 `-Progress <步骤名>`，只沿用同一项目、Target 与已登记的设计身份（`fileId` / `layerId` / `ui` / `designPageName`）；身份变化或登记表缺失时从 `fetch` 新开运行，不得用续跑把旧产物重新标成另一页。标题、译文、术语表与图标命名仍可作为语义输入修改。
