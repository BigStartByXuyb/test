...
- **区域前缀（`ui`）不再写死 F2**：`run-all.ps1` 按固定顺序取值——① 命令行 `-Ui`；② 项目登记表 `pages[].ui`；③ 项目登记表 `derivation` 里的 `F<n>`；④ Target 的编号前缀（`F2ManualAlign` → `F2`）；⑤ 没有编号时取 Target 的首个英文词（`HomeContent` → `Home`、`Home` → `Home`，即"外层的语义英文"）；⑥ 都取不到直接报错。它写进采集快照的 `ui` 字段并决定宿主壳目录 `UI/<区域>/View|ViewModel`，因此**不允许静默默认值**（写死 F2、不报错、不回显来源那种）；上面这条取值链是**有规则、可复核的解析**——run-all 会把命中的来源一起回显（`Ui: Home（来源: Target 首词（HomeContent））`），取不到就报错。`mastergo-dsl-pipeline.ps1` 单独调用时必须显式给 `-Ui`。
