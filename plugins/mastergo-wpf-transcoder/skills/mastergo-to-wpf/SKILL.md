...
- **作业 A 的差异**（步骤号与名称不变，步内命令与产物不同）：第 5 步 = 共享类型判定（只读共享类型表，产出 `Generated/<Target>.component-types.json`）；第 8 步 = 共用 Layout 清单推导 **+** 布局产物推导 `Generated/<Target>.wpf-layout.json`（分区 → 行列 → 格子；格子登记设计稿尺寸与控件在格内的偏移）；第 10 步 = 真控件 `View.xaml`（含本页 Icon 字典合并点）+ 宿主壳 + 本页 Icon/语言字典 + Layout 注册，**不发射 IOContorl 页面 XML**；第 11/12 步 = 布局门禁（越界 / 锚点格冲突 / 禁止类型 / 尺寸来源 / 协议 / 资源键 / 硬编码文本 / 尺寸约束一致性与落格 / 格子尺寸与尺寸·对齐发射；失败与提示的逐条口径见 `references/adapters/mw-wpf/mw-wpf-mode.md` 第 5 节与 `references/adapters/mw-wpf/page-build-rules.md` 第 4 节，此处不复述）。第 1–4、6、7、9 步两条路线沿用同一套。
...
- **布局门禁**：`scripts/adapters/mw-wpf/check-wpf-layout.js`（条目清单、失败与提示口径见 `references/adapters/mw-wpf/mw-wpf-mode.md` 第 5 节与 `page-build-rules.md`，本节不复述）
