...
- **节点发射顺序按设计稿上下布局**：同一父节点下的子节点按设计坐标 **Top（Y）主序 → Left（X）次序** 发射（坐标相同保持 mapping 原顺序），与 `mapping.nodes` 的图层树顺序无关。只改排列顺序，`ID` / 坐标 / 属性 / 层级关系都不变。上下布局容器的子节点顺序决定运行时显示顺序，因此禁止按 DSL 图层顺序发射。排序唯一真值源：`gen-iocontrol-xml.js` 的 `sortNodesByDesignOrder()`；fresh 与 merge 共用。
