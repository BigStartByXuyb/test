...
- `audit-mtslg-feishu-map.js`：组件映射文档或模板 JSON 修改后运行，用于检查文档覆盖（`missing` / `unregisteredVariants` / `undocumented` / `duplicateMatchKeys` 必须全为空），不是页面生成步骤；新增或修改映射的整批同步清单见 `skills/mastergo-iocontrol-document-format/SKILL.md` 的「新增/修改映射的同步清单」。
