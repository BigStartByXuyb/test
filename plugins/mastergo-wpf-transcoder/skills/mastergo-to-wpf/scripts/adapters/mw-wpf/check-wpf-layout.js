...
//   R12 尺寸约束未落格：本页要发射的可见带约束节点在布局里没有对应格子即失败。成因在设计输入——
//       例如容器子树里没有可发射的控件，推导给不出格子（不是产物被改坏的判据）；
//       本页不发射的节点（页面根 / 不可见 / 框架固定区，布局产物的 constraintExempt）只登记提示，不失败
...
function checkConstraints(layout, dslConstraints, xamlText) {
  ...
  Object.keys(dslConstraints).forEach(function (ref) {
    ...
    report("R12", ref, "本页要发射的节点在 DSL 里带尺寸约束，但布局产物里没有对应格子（例如容器子树里没有可发射的控件，推导给不出格子），约束未落到产物");
  });
}
