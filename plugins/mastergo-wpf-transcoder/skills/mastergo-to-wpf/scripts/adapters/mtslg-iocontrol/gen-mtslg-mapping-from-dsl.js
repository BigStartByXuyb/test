...
function walk(node, parentRef, pageAbsX, pageAbsY) {
  ...
  // 文本对齐：设计稿把左/右/居中对齐放在 TEXT 节点级 textAlign 字段（取值 left / right / center）。
  // 这里只做机械提取与归一，取值口径（哪一种是 Left、缺失怎么办）在 addText 按映射表 textBlockAlign 执行。
  if (typeof node.textAlign === "string" && node.textAlign.trim()) {
    textAlignByRef.set(node.id, node.textAlign.trim().toLowerCase());
  }
...
// ID = "MX_" + sha256(页面键 + "\n" + 节点 ref) 前 32 位小写十六进制。
//   · 页面键 = DSL 根节点自己的 id（= 设计帧的 layerId，形如 "123:4567890"）：同一份设计稿恒定，
...
function addNode(sourceRef, controlType, attrs, options = {}) {
  ...
  if (defaultLeft === null) throw new Error("TextBlock 的 Left 无法计算（缺 pageAbsX 或内容区原点X）: " + sourceRef);
...
//   · 命中模板族但结构部分命中的节点（如表格表头可见文本不足）已在各自族处理里登记 pending，保持隔离；
//   · 纯布局包裹层（FRAME / GROUP / LAYER，既未命中模板族也未登记 pending）→ **展平**：容器本身不发射控件，
