...
// 文档侧解析：只认结构化写法（不猜散文）。产出的每个 token 带 `kind`，由 classifyDocTokens 判去留：
//   kind=variant —— 文档明确写出的**变体值**（`MasterGo 变体：…`、`待确认变体：…`、`属性 1=` /
//                    `按钮类型=` / `变体=` 段）：解析不到映射表就是文档凭空多写的，退 2；
//   kind=section —— **章节名**（`组件集=` / `聚合集合=` / `独立组件=` 段）：解析不到映射表算标签，不算错
//                    （「选型标题」本来就可能不等于任何一个变体名，如《单选+多选》）。
