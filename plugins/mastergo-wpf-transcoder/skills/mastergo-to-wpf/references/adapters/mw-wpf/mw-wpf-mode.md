...
3. **尺寸照设计稿**：普通 region 的行列尺寸取设计稿像素（`source: "design"`）。声明了 flex 主轴的容器按**条目带 + 间隙带**显式成带：
    ...
    - **列宽**（主轴 `row`）：固定项照设计稿像素——固定项 = 子树里只有相机控件的条目（相机所在的 Grid），或区域根网格里贴主轴末端的最末条目（页面常驻右栏）；其余**容器**条目自适应（`<ColumnDefinition />`；主轴 `row` 上出现多个自适应条目时按设计稿比例写加权星号 `1016*`），叶子控件照设计稿像素；
...
7. **星号写法**：单个自适应条目写成裸 `<RowDefinition />` / `<ColumnDefinition />`；主轴 `row` 上出现多个自适应条目时按设计稿比例写加权星号（`Width="1016*"`，见第 3 条；行方向（交叉轴）只会出现收尾的那条裸星号，没有自适应条目）；像素尺寸写 `Height="<值>"`，间隙带写 `Auto`。
