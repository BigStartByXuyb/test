...
1. **分区**：只有「框架固定区（顶部栏 / 底部栏，`emit=false`，按框架 Token 高度）」与**一个内容区**（`emit=true`）。框架常驻区（顶部栏、底部栏、右侧栏常驻，见下条）不进页面，所以**页面外层只有唯一一个 Grid —— 内容网格本身**（不再另套一层带 `Grid.Row` 的根 Grid）；设计稿的业务内容（含容器链条）全部落在同一个内容区里，内部再按行列分格。
