...
3. **尺寸照设计稿**：普通 region 的行列尺寸取设计稿像素（`source: "design"`）；**格子尺寸 − 控件尺寸 = 间距**。格子尺寸由推导登记在格子上（像素带照值、唯一的收尾星号带取"可用尺寸 − 其余像素带"、跨格按 `rowSpan` / `columnSpan` 累加；内层网格的可用尺寸是承载物自身设计尺寸），控件写自身设计稿尺寸（`Width` / `Height`），差值落在哪一侧由格上的设计稿偏移（`offsetX` / `offsetY`）决定：贴起始边（差值全在末端＝间距，最常见）／贴末端／两侧相等居中／非对称内缩＝贴起始边 + `Margin` 补偏移。唯一实现在 `scripts/lib/design-box.js`；格子尺寸与控件尺寸相等时不写尺寸也不写对齐；推导为避让撞格而下移的格子（`cell.shifted`）没有偏移真值，只写控件自身尺寸、不表达间距 / 对齐；算不出正数格子尺寸的那一维（`cell.unsized = {width?,height?}`：收尾星号带被前面的像素带吃光，内容溢出了承载物）什么都不写，另一维照写。这两类格子的格子尺寸与承载物设计尺寸照常登记与核对（`unsized` 的那一维只有在重算确实非正数时才放行）。格子放不下时是设计问题，不改写成"凑得下"的值。
