...
1. **禁止写法**：发射区出现写法表未登记或登记为待确认的类型（`Border`）即失败；框架固定区里出现控件也失败。`Camera` 是 `manual-only`（仅用户确认），可以发射。
2. **协议属性名**：`--types` 产物节点的 `attrs` 里，凡属协议属性名集合（由写法表 `protocols` 派生）的键，必须在该类型的 `protocols` 里登记过（只校验名字，不校验取值形状）。步骤 8 传类型判定产物，步骤 11/12 传 mapping 定稿。
...
10. **待人工确认（提示）**：页面用到写法表 `status: manual-only` 的类型（如 `Camera`：只有用户确认、没有手册条目与真实页面证据）时，登记提示 `R10`，不失败——"首次生成需人工确认、事后回填手册"以提示为落点。
