...
1. **禁止写法**：发射区出现写法表未登记或登记为待确认的类型（当前是 `Border`）即失败；框架固定区里出现控件也失败。（`Camera` 已登记为 `manual-only`，发射 `<s:Camera>`，不再属于本条。）
2. **协议语法**：协议属性必须符合写法表 `protocols` 的形状；出现未登记的协议属性失败。
3. **格子越界**：`row` / `column` / 跨格数必须落在本 region 的行列范围内。
4. **空行空列**：没有任何格子覆盖、也不是被星号撑开的收尾行/列失败。
5. **同格冲突**：同一格多控件必须各自带互斥条件。
