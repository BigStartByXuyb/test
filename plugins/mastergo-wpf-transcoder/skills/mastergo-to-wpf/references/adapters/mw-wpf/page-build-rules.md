...
14. **格子尺寸与尺寸/对齐发射**：每个格子必须登记格子尺寸（像素带照值、唯一的收尾星号带取"可用尺寸 − 其余像素带"、跨格按 span 累加）与承载物设计尺寸；发射报告里该格子的 `Width / Height / HorizontalAlignment / VerticalAlignment / Margin` 必须与 `scripts/lib/design-box.js` 的同一实现一致（格子尺寸 − 控件尺寸 = 间距，差值落在哪一侧由设计稿偏移决定）。缺格子尺寸、格子尺寸与行列定义重算不一致、报告缺条目或取值不一致都失败（入参前提见 `check-wpf-layout.js` 头部 CLI 注释）。**例外**：推导为避让撞格而下移的格子（产物里的 `cell.shifted`）不是设计稿那条带，没有偏移真值——只写控件自身尺寸、不写对齐；门禁只把「缺偏移真值」这一半（尺寸 / 间距对齐）按提示登记，发射报告缺条目、取值不一致与第 11 / 13 条照常失败。
