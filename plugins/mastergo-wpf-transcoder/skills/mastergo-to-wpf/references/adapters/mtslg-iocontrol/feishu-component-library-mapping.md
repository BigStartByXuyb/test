...
- **空白处理的两条口径**：页面 XML 的属性文案只做换行归一、其余空白原样保留；字典值则额外做「行内空白折叠成单个空格 + 行首行尾 trim」（设计稿的左右留白是排版产物，字典值去掉它），因此两者允许在空白上不同。键派生、译文查找、术语表匹配用「压平值」（所有空白折叠成单个空格并 trim），与上述两者都不同。
...
- **实现真值源**是 `scripts/lib/script-helpers.js`（换行归一、属性/元素转义、比对归一四函数的唯一实现）；映射表 `textNewlinePolicy` 只**登记**同一口径（码点集合、归一目标、两种发射写法）供人读与回归断言比对，不是脚本的运行期输入。改口径时先改实现，再同步映射表登记块与三份人读文档。
