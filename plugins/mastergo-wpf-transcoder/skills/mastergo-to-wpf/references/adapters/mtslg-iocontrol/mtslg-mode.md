...
（续跑身份提示：本表是**新开运行**解析身份的输入；续跑回放运行登记表 `Generated/runs/<Target>/run.json` 里冻结的 `identity`，因此改动本表的 `pages[].ui` / `pages[].designSource.*` 不会影响正在续跑的这一次——要改身份请新开一次运行。口径见 `bundle-manifest.md` 第 7 节。）
