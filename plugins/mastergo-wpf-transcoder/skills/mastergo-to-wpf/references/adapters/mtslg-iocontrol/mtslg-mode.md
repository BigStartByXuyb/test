...
- **`Left` 的口径随 `Align` 切换（同一个属性名，两个含义）**：`Align=Left`（含设计稿 `center`）时 `Left` = 控件**左边缘**到输出父容器**内容区左边缘**的距离（与既有口径一致）；`Align=Right` 时 `Left` = 以控件**右上角**为原点，量到输出父容器**外框右边缘**的距离 = `父外框右边缘 − (控件 pageAbsX + 设计稿 bbox 宽度)`。外框右边缘：页面根 = **页面宽度**（根节点宽度）；容器 = **容器 pageAbsX + 容器宽度**（外框，不是内容区右边缘）。宽度来源固定为**设计稿 bbox 宽度**（mapping `nodes[].dslWidth`）——产物 XML 的 `Width` 仍是 `NaN`（自适应），它只参与这里的 `Left` 计算。两条口径的**运行期实现只有一处**：`scripts/lib/script-helpers.js` 的 `textBlockLeftValue` / `parentOuterRightEdge`（映射生成器 / 容器重挂 / XML 渲染 / provenance 校验直接调用；右对齐分支另由 mapping 节点的 `leftBasis` 标识判定）。坐标核对链路按节点表口径核对：右对齐节点由 `check-coords.mjs` 用同一实现算出 `expectedLeft` 显式给出，其余节点仍是 `x − contentOriginX`。口径登记在映射表 `textBlockAlign.distance*`（登记 + 回归断言，不是运行期输入）。
