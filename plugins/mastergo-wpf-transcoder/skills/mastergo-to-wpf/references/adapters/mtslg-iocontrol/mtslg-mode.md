...
- **关于 `Value` 的运行时刻画，本插件唯一能自证的是「恒写该属性」**：`ControlType="DataGrid"` 必须声明并**恒写** `Value`（依据：`controlTypeRequiredAttrs.DataGrid` 含 `Value`，生成器恒写、禁止省略该属性；省略会让属性缺失，而缺失与空串是两种不同的产物状态）。注意这里的限定只针对运行时刻画——下面三态条目里的 `valuePending` 标记以及状态②的「必须同时置位并写进审计与交付说明」属于**本插件可机械核对的门禁要求**，不是待目标项目确认的运行时刻画，实施者与复核者都不得把它降级为提示性说明。
