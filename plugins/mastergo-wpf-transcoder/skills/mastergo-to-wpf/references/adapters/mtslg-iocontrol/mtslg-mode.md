...
**项目登记表（`docs/page-registry.json`）的字段口径**（`run-all.ps1` 会读它）：选页顺序是「`-Target` 命中 → `-LayerId` 命中 → 登记表只有一页时用它」，**登记表有多页时必须用 `-Target` 或 `-LayerId` 命中本次页面**——没命中就直接报错，不取第一页顶上（否则会拿别的页面的 fileId/layerId/ui 当成本次来源）：
