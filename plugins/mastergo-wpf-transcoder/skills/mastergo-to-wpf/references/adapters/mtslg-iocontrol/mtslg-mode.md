...
7. 【仅运行时交付门禁】`sync-to-mt.ps1` 按适配记录同步到唯一已确认的运行配置目录，前置条件是静态 XML、来源、坐标与键查证全部通过**且用户要求部署到运行目录**（与 `SKILL.md` 脚本索引同口径）；同步前强制备份。用户未要求运行时交付/部署时，本步与第 8 步一并跳过，纯静态交付在第 6 步结束。
...
5. **风险闸**：Target→文件名映射部分在闭源代码，首次加载验证属「项目运行时交付」门禁（只在用户明确要求运行时交付时执行，口径同第 0 节 Target 映射条）；纯静态结构映射交付时登记为待确认项、不执行加载。验证失败则回退「改现有页面」路径并报告。
