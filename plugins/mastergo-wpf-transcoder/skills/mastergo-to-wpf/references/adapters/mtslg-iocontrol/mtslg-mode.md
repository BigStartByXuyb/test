...
7. 【仅运行时交付门禁】`sync-to-mt.ps1` 按适配记录同步到唯一已确认的运行配置目录，前置条件是静态 XML、来源、坐标与键查证全部通过**且用户要求部署到运行目录**（与 `SKILL.md`「辅助脚本触发矩阵」的 `sync-to-mt.ps1` 条目同口径）；同步前强制备份。同步是部署动作，宿主加载验证发生在同步之后的第 8 步。用户未要求运行时交付/部署时，本步与第 8 步一并跳过，纯静态交付在第 6 步结束。
...
4. 坐标核对 → 【仅运行时交付门禁】同步 → 执行目标项目已确认的重载动作；用户未要求运行时交付时到此为止，改为登记待确认项（口径同 7.2 第 7、8 步）。
