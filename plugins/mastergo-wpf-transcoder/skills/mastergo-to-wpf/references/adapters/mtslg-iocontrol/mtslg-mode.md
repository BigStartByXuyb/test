...
- **待确认项 TD-065**：上一段的容器内容区原点口径依赖「GroupBox 未显式指定 `Style` 时框架落到哪个隐式默认样式、其内容区边框与标题条高是否等价于 `{left:1, top:35}`」。框架源码不随本仓库分发，无法在仓库内核验，已登记为 `references/adapters/mw-wpf/framework-manual/05-best-practices/pending-confirmations.md` 的 **TD-065**；确认前按上一段口径换算，回填后按该台账行登记的同步清单执行（清单只在台账写一份，本处不重复枚举）。
