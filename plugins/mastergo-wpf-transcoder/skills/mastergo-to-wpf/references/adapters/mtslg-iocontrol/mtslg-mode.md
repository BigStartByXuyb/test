...
- **`FontWeight` 命中才写**：`TextBlock` 的 `FontWeight` 只在设计稿字重**不是 normal** 时发射，**值取设计稿自己的字体样式名**（`dsl.styles[<text[0].font>].value.style` 里的 `fontStyle`，例如 `Bold`、`SemiBold`）——不是把数值翻译成名字。带字体族档位数字的前缀要去掉（`75 SemiBold` → `SemiBold`、`55 Regular` → 判为 normal）；`Regular`/`Normal`/`Book`/`常规体`/`常规` 以及 `weight=400` 一律视为 normal、**不写该属性**（不写空串，也不参与 `controlTypeRequiredAttrs` 恒写集合）；样式名缺失时才回退写 `weight` 数值。规则真值源是映射表 `textBlockFontWeight`（`normalStyleNames` + `normalValues`），`FontSize` 的恒写规则不受影响。输入框、选择框等非 TextBlock 控件按正式变体模板取自身宽高；非 TextBlock 控件的文本不发射 `FontWeight`。
