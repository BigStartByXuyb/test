...
- **变体清单**：`layoutRules.bottomBar.variants` 共 16 个，**逐个列全**——首页-长方形、非首页-长方形、方-icon+文案、方-icon、非首页-F、非首页-文案、非首页- F、DI 显示-0、DI 显示-1000、DI 显示-2000、DI 显示-3000、DI 显示-4000、DI 显示-5000、DO 显示-0、DO 显示-1000、DO 显示-2000。（本行由门禁与映射表的 `variants` 一一核对：总数和每个名字都要逐字对上，所以不要把 DI / DO 的多个变体压成一行简写——那样映射表新增变体时会漏网。）其中 `DI 显示-*` / `DO 显示-*` 是 84×84 的图标+文案按钮：不写 `TopLeftContent`（没有 F 键槽位），图标按图标映射发射，`Name` 照设计稿文本写入。
...
**文案属性的换行**：`Name`（菜单名）与 `TopLeftContent` 等文案属性若含换行，一律写字符引用 **`&#x0a;`**——与页面 XML 同一条换行口径：设计换行（`U+2028` 行分隔符 / `U+2029` 段分隔符 / `CR`(U+000D) / `LF`(U+000A) / `CRLF`）先统一归一成 LF，再按框架写法发射；**属性里不能出现字面换行**（XML 解析器会把它归一成空格）。发射由共享的 `xmlAttr` 保证（实现真值源在 `scripts/lib/script-helpers.js`），Layout 生成器不另写一份转义。
