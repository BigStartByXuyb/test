...
**哪些图形要进本页台账（判定表）**：判据是「该图形是否被本页某个 Icon 槽位引用」，但**实例归属哪一族，决定去查哪一套登记**——映射表里有两族，各有各的键，不要混用：

| 实例属于哪一族 | 怎么认出它 | 图标要不要登记 |
|---|---|---|
| **组件模板族**（`componentTemplates` / `mainMenuTemplates` / `rightSidebarTemplates` / `rightSidebarComponentTemplates` / `cameraTemplates` / `inputTemplates` / `infoGroupTemplates` / `tableTemplates` / `textTemplates` …） | 按**该族自己 `match` 块登记的键**匹配——**各族不同，不要按"族"概括**：`componentTemplates` / `mainMenuTemplates` / `inputTemplates` / `selectBoxTemplates` 用 `property:"属性 1"`；`rightSidebarTemplates` 用 `property:"按钮类型"`；`textTemplates` 用 `property:"role"`；`rightSidebarComponentTemplates` / `cameraTemplates` / `infoGroupTemplates` 用 `componentSet:true`（按组件集名）；`tableTemplates` 用 `structural`（结构签名） | 看该实例命中的**变体登记的 `iconPolicy`**：`single-path` → 是；`pair-per-button` → 是（一按钮两图形各自成条）；`runtime` → 否（`Icon` 是目标项目已有资源键，本页不生成，台账也不该有，有也会被 Bundle 剔除并记进审计 `runtimeIcons`，例：ENTER / EXIT）；`none` → 否 |
...
判定顺序：先认这个图形所属实例属于哪一族（组件模板族 / 布局族 / 常驻分组 / 宿主 / 相机），再按对应一列判，最后看它是否落在第 5 节的 omit 角色里。注意 `discover` 的候选清单只给**可机械判定**的归属标记（`ownerControlType` 仅覆盖 IconButton / Camera 这类），**底部栏菜单图标不带归属标记**——它们的结论来自本表，不要因为没有标记就跳过，也不要靠图层名（"顶部状态栏""键盘""组 2178"）猜。
