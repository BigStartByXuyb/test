...
4. **断点续跑（身份不可变）**：`run-all.ps1 -Progress <步骤>` 且步骤号大于 1 时才用 `run-registry.mjs init --keep` 沿用同一份登记表（`-Progress fetch` 即第 1 步不带 `--keep`，会新开一次运行：新 `runId`、产物登记清空；登记表缺失时直接报错并提示从 `fetch` 新开）。续跑**回放首次运行冻结的身份**：`fileId` / `layerId` / `ui` / `designPageName` 一律取 `run.json` 的 `identity`，不再按项目登记表重新推导。`target` / 归一化后的 `projectRoot` / schema 由守卫直接校验，失败发生在保存之前、原登记表逐字节不变；不得通过编辑 `run.json` 绕过。
   - **省略 → 用冻结值**：capture 拿到的一定是首次那份身份，不会退回 DSL 根节点名，也不受项目登记表后续增删改影响；冻结值为空而本次解析出非空的，按「不得补写」失败。
