...
本文件定义两套适配器共用的语义边界与跨适配器规则（公共匹配原则、映射登记形态、TextBlock 尺寸与顶部示例标题），不重复维护具体 XML 模板或 WPF 控件细节。

## 公共匹配原则

- 先识别正式组件实例、独立组件集名称、公开变量属性和真实变量值；**“父节点语义”不是匹配键**——任何适配器都不得把父节点语义当作命中条件（父节点链用于定位实例、读取证据与回溯，也用于确认真实结构、裁剪边界与来源），也不得凭截图、Group 名称或外观猜测。这里禁止的只是“拿父节点语义当匹配键”；按设计稿真实的父子结构读取槽位、坐标与裁剪边界是允许的，也是必须的。
