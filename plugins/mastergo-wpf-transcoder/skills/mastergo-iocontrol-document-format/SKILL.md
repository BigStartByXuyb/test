...
**一个族一个键**：模板族实际用哪个键，登记在共享类型表 `skills/mastergo-to-wpf/references/component-types.json` 的 `match` 字段里，**一个模板族只登记一个键、解析时也只用一个**——键的种类/属性名一律以共享类型表各族的 `match` 为准，本规范不另立清单；本规范里出现的具体键取值/属性名（如 `componentTemplates.match.property` = `属性 1`）只为例示、须与共享类型表同步。映射文档里每个族的“匹配规则”必须写明它用的是哪一种，并与映射表登记一致；不要写成“多个键任选其一”或“先按属性、取不到再按名字”这类多候选兜底。另一种键 `componentName: true`（按被引用组件的名字命中）只用于 Layout 层的底部栏，**不是组件模板族可用的键**。
...
- 禁止使用无法判断命中范围的裸标题 `### 固定模板`，也不使用 `### 固定模板：父节点=…` 这类"父节点语义"标题（映射表的模板族里没有按父节点分流的字段；父节点链只用于定位实例、读取证据与回溯）。**历史文档里若存在这类标题，必须在本次改动里改掉**——覆盖审计会把它的取值报进阻断字段 `unregisteredVariants`（脚本以退出码 2 结束），不会容忍"留着下次再说"。改写目标分两种，别混用：优先改成**真实变体值标题**（`属性 1=` / `按钮类型=`）；要按组件集组织章节时用**章节名** `组件集=`，它必须与同一章节里**至少一个能解析成变体的标题或 `MasterGo 变体：` 行**成组出现（章节名本身不是变体值，要靠同组变体作锚点），否则会被当成孤儿章节。该族的匹配键仍按映射表的 `match` 登记执行。

## XML 书写规则

- `ControlType`、节点数量、父子关系和槽位顺序由固定模板决定。
- `Value`、`IOName`、`IOCommand`、`IOEnable`、`IOState`、`LangName`、`Left`、`Top`、`Width`、`Height`、`FontSize`、`Align`（文本对齐，恒写）等由对应 MasterGo 节点或已确认配置填充；其中 **`Align=Right` 时 `Left` 的口径不同**（以控件右上角为原点量到父容器外框右边缘的距离，口径见组件库映射文档与映射表 `textBlockAlign.distance*`）。
...
1. **机器真值源（拆成两处，都以插件根为基准）**：① **共享类型表** `skills/mastergo-to-wpf/references/component-types.json`——模板族结构、`match`（一族一键，键的种类以各族 `match` 为准；`componentName` 只用于 Layout 层的底部栏）、`variants` 真实属性值，以及跨路线共用的 `layoutRules`；② **本路线写入规则** `skills/mastergo-to-wpf/references/adapters/mtslg-iocontrol/mtslg-iocontrol-map.json`——`controlTypeRequiredAttrs` 必写字段、按钮族 `iconSize` 等 IOContorl 专属写法；它用 `extends` 指向共享类型表。同一个「匹配属性名 + 属性值」只能登记在一个模板族。两个文件都只经 `scripts/lib/load-template-map.js` 读取（合并后的形状与拆分前一致），脚本不得自行 `JSON.parse`。**新增的是"族"（带 `variants` 的新 `*Templates`）时，下面两项一并完成，缺一项就不算同步完成**：① 在 `skills/mastergo-to-wpf/scripts/adapters/mtslg-iocontrol/gen-mtslg-mapping-from-dsl.js` 写出该族的发射分支（`match.family === …`），并把族名登记进该文件的 `SUPPORTED_TEMPLATE_FAMILIES`；② 跑 `skills/mastergo-to-wpf/scripts/tests/template-family-coverage.test.js`——它与映射表**双向**比对族集合，并逐个确认非兜底族真有发射分支（只登记不写分支同样失败）。覆盖审计看不到生成器这一层，所以它不能替代这两项。
