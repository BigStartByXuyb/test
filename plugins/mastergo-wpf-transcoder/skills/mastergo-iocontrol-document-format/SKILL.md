...
**一个族一个键**：模板族实际用哪个键，登记在 `mtslg-iocontrol-map.json` 的 `match` 字段里，**一个模板族只登记一个键、解析时也只用一个**——键的种类以映射表各族的 `match` 为准，本规范不另行维护属性名清单（当前出现的种类：公开属性名 `属性 1` / `按钮类型`；`componentSet: true` 按组件集名命中，如右栏独立组件族 `rightSidebarComponentTemplates`；表格族的结构签名 `structural`）。映射文档里每个族的“匹配规则”必须写明它用的是哪一种，并与映射表登记一致；不要写成“多个键任选其一”或“先按属性、取不到再按名字”这类多候选兜底。另一种键 `componentName: true`（按被引用组件的名字命中）只用于 Layout 层的底部栏，**不是组件模板族可用的键**。
...
- 禁止使用无法判断命中范围的裸标题 `### 固定模板`，也不使用 `### 固定模板：父节点=…` 这类"父节点语义"标题（映射表的模板族里没有按父节点分流的字段；父节点链只用于定位实例、读取证据与回溯）。**历史文档里若存在这类标题，必须在本次改动里改掉**——覆盖审计会把它的取值报进阻断字段 `unregisteredVariants`（脚本以退出码 2 结束），不会容忍"留着下次再说"。改写目标：真实变体值标题优先（`属性 1=` / `按钮类型=` / `组件集=`）；`组件集=` 需要与同一组件集章节里**至少一个能解析成变体的标题或 `MasterGo 变体：` 行**成组出现（它是章节名，要靠同组变体作锚点），否则会被当成孤儿章节。该族的匹配键仍按映射表的 `match` 登记执行。
