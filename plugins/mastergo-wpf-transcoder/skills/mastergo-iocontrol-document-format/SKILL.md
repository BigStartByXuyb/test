...
- 禁止使用无法判断命中范围的裸标题 `### 固定模板`，也不使用 `### 固定模板：父节点=…` 这类"父节点语义"标题：父节点语义**不是匹配键**（映射表的模板族里没有按父节点分流的字段），它只用于定位实例、读取证据与回溯。历史文档里若存在这类标题，新增或修改时改成 `组件集=` / `结构分支=` / 真实变体值标题；该族的匹配键仍按映射表的 `match` 登记执行。
